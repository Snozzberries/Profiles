...
if (-not $isAdmin)
{
    ...
    if (-not (Test-Path $env:TEMP\ohmyposh.json)) {
        irm https://raw.githubusercontent.com/Snozzberries/Profiles/main/ohmyposh.json -OutFile $env:TEMP\ohmyposh.json
        oh-my-posh init pwsh -c $env:TEMP\ohmyposh.json|Invoke-Expression
    } else {
        oh-my-posh init pwsh -c $env:TEMP\ohmyposh.json|Invoke-Expression
        Start-Job -Name "Downloading Oh My Posh Profile" -ScriptBlock {
